...
class AzureBlobFiles extends Files {
  ...
  async getFileInfo (filePath) {
    ...
    const blob = await this._wrapProviderRequest(azureProps.blockBlobURL.getProperties(this._azure.aborter),
      { filePath }, filePath)
    const blobDesc = {
      name: filePath,
  ...
  async _listFolder (filePath) {
    const __listFolder = async (_filePath, azureProps) => {
      ...
      do {
        ...
        if (_filePath !== '') {
          options.prefix = _filePath
        }
        const response = await this._wrapProviderRequest(azureProps.containerURL.listBlobFlatSegment(this._azure.aborter, marker, options),
          { filePath: _filePath })
        marker = response.marker
        elements = elements.concat(response.segment.blobItems.map(blob => {
          const blobDesc = {
            name: blob.name,
