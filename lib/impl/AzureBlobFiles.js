...
class AzureBlobFiles extends Files {
  ...
  async _listFolder (filePath) {
    const __listFolder = async (_filePath, /** @type {AzureProps} */azureProps) => {
      ...
      // todo error handling and debug logging => wrap list request (but doesn't return a promise..)
   ...
   * @private
   */
  _getMonitoringDetails (res, e) {
    const getAzureRawHeaders = (obj) =>
      obj.headers && typeof obj.headers.rawHeaders === 'function'
        ? obj.headers.rawHeaders()
        : obj.headers
    ...
    // not an error + response has no response object
    if (!e && typeof res === 'object' && !res._response && !res.originalResponse) {
      return {
        response: {
          requestId: res.requestId
        },
        request: {
          requestId: res.clientRequestId
        }
      }
    }
    let rawResponse = {}
    // azure sdk doesn't return response object in the same way for all operations :(
    if (!res && typeof e === 'object') {
      rawResponse = e.response
    } else if (res && res._response) {
      rawResponse = res._response
    } else if (res && res.originalResponse) {
      rawResponse = res.originalResponse._response || {}
    }
    const response = {
      headers: getAzureRawHeaders(rawResponse),
      status: rawResponse.status,
      body: rawResponse.body
    }
    const request = rawResponse.request && {
      ...rawResponse.request,
      headers: getAzureRawHeaders(rawResponse.request),
      // remove huge objects
