...
const codes = require('../FilesError').codes
...
class AzureBlobFiles extends Files {
  ...
  constructor (credentials) {
    ...
    if (res.error) {
      if (typeof credentials === 'object') {
        // remove secrets
        if (credentials.sasURLPrivate) credentials.sasURLPrivate = credentials.sasURLPrivate.split('?')[0]
        if (credentials.sasURLPublic) credentials.sasURLPublic = credentials.sasURLPublic.split('?')[0]
        delete credentials.storageAccessKey
      }
      throw new codes.ERROR_BAD_ARGUMENT({ messageValues: [res.error.message], sdkDetails: cloneDeep(credentials) })
    }
  ...
  async _createReadStream (filePath, options) {
    const fileProps = this._propsForPath(filePath)
    return (await this._wrapProviderRequest(fileProps.blockBlobURL.download(this._azure.aborter, options.position || 0, options.length), { filePath, options }, fileProps.path)).readableStreamBody
  }
