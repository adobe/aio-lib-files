...
const DEFAULT_CDN_STORAGE_HOST = 'adobeio-static.net'
...
class AzureBlobFiles extends Files {
   ...
   * @param {object} config OW config
   * @memberof AzureBlobFiles
   * @private
   */
  constructor (credentials, config) {
    super()
    this.config = utils.withHiddenFields(config)
   ...
   * @memberof AzureBlobFiles
   */
  static async init (credentials, config) {
    const azureFiles = new AzureBlobFiles(credentials, config)
  ...
  async _getPresignUrl (filePath, options) {
    const tvm = await utils.getTVMClient(PROVIDER, this.config)
    const presignOptions = Object.assign( { blobName: filePath }, options)
    const cred = await utils.getAzureBlobPresignCredentials(tvm, presignOptions)
