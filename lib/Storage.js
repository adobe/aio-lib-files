...
class Storage {
  ...
  static _isRemoteDirectory (filePath) {
    const normalized = Storage._normalizeRemotePath(filePath)
    return normalized.endsWith('/') || normalized === '' || normalized === Storage.publicPrefix
  }
  ...
  async _wrapProviderRequest (requestPromise, filePath) {
    return requestPromise.catch(e => {
      ...
      if (filePath && status === 404) throw new StorageError(`remote file '${filePath}' does not exist`, StorageError.codes.FileNotExists, e)
      if (status === 403) throw new StorageError(`access forbidden, make sure your credentials are valid`, StorageError.codes.Forbidden, e)
