...
class AzureStorage extends Storage {
   ...
   * @property {string} [containerName] name of container to be used. Another
   * `${container}-public` container will additionally be used.
   *
   * @property {import('../Storage').RemotePathString} path normalized path
   */
  ...
  static async init (credentials) {
    ...
    if (!azureStorage._azure.sasCreds) {
      ...
      await createContainerIfNotExists(azureStorage._azure.containerURLPrivate, azureStorage._azure.aborter)
      await createContainerIfNotExists(azureStorage._azure.containerURLPublic, azureStorage._azure.aborter, true)
    }
  ...
  async _fileExists (filePath) {
    const fileProps = this._propsForPath(filePath)
    try {
      await fileProps.blockBlobURL.getProperties(this._azure.aborter)
    } catch (e) {
      if (e.response.status === 404) return false
      throw new StorageError(`Error response from provider, status: ${e.response.status}`, StorageError.codes.Unknown)
    }
    return true
  }
  ...
  async _deleteFile (filePath) {
    const props = this._propsForPath(filePath)
    await props.blockBlobURL.delete(this._azure.aborter)
    return props.path
  }

  async _createReadStream (filePath, options = {}) {
    const fileProps = this._propsForPath(filePath)
    return (await fileProps.blockBlobURL.download(this._azure.aborter, options.position || 0, options.length)).readableStreamBody
  }
  ...
  async _createWriteStream (filePath) {
    ...
    const AzureBlobWriteStream = class AzureBlobWriteStream extends stream.PassThrough {
      constructor (fileProps, azureAborter) {
        super()
        this.promise = azure.uploadStreamToBlockBlob(
          azureAborter,
          this,
        ...
        ).then(_ => fileProps.path) // Return value
      }
    ...
    return new AzureBlobWriteStream(this._propsForPath(filePath))
  }
  ...
  async _writeBuffer (filePath, content) {
    ...
    await fileProps.blockBlobURL.upload(this._azure.aborter, content, content.length, uploadOptions)
  ...
  async _copyRemoteToRemoteFile (srcPath, destPath, options) {
    ...
    await destProps.blockBlobURL.startCopyFromURL(this._azure.aborter, srcProps.blockBlobURL.url)
    return destPath
  }
