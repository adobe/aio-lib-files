  ...
  async copy (srcPath, destPath, options = {}) {
    ...
    const _getFiles = async (filePath, isLocal = false, localFileStats) => {
      if (isLocal) {
        ...
        if (!localFileStats.isFile() && !localFileStats.isDirectory()) throw new FilesError(`file '${filePath}' is not a valid file or directory (maybe a symlink?)`, FilesError.codes.BadFileType)
    ...
    const _getLocalFileStats = async (filePath) => {
      ...
      if (stats && !stats.isDirectory() && !stats.isFile()) throw new FilesError(`file '${filePath}' is not a valid file or directory (maybe a symlink?)`, FilesError.codes.BadFileType)
      return stats
    }
    ...
    validateInput(options, joi.object().keys({ noOverwrite: joi.boolean(), localSrc: joi.boolean(), localDest: joi.boolean(), progressCallback: joi.func() }).oxor('localSrc', 'localDest').label('options').required())
    ...
    if (localDestStats && localDestStats.isFile() && srcIsDirectory) {
      throw new FilesError(`local dest '${destPath}' is a file but src is a directory, cannot copy to file.`, FilesError.codes.BadFileType)
    }
    ...
    if (allSrcFiles.length === 0) throw new FilesError(`src '${srcPath}' does not exist`, FilesError.codes.FileNotExists)
