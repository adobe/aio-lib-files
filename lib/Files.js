...
class Files {
  ...
  static _normalizeRemotePath (filePath) {
    ...
    logger.debug(`normalized path: '${filePath}' to '${res}'`)
    return res
  }
  ...
  static _isRemotePublic (filePath) {
    const isPublic = filePath.startsWith(Files.publicPrefix)
    if (isPublic) logger.debug(`'${filePath}' is public`)
    return isPublic
  }
  ...
  static _isRemoteDirectory (filePath) {
    const isDir = filePath.endsWith('/') || filePath === '' || filePath === Files.publicPrefix
    if (isDir) logger.debug(`'${filePath}' is a directory`)
    return isDir
  }
  ...
  async copy (srcPath, destPath, options = {}) {
    ...
    const _normalize = (filePath, isLocal = false) => {
      if (isLocal) {
        const normalized = upath.toUnix(upath.resolve(filePath)) + (filePath.endsWith('/') ? '/' : '') // keep trailing / important in case local dest doesn't exist
        logger.debug(`normalized local path: '${filePath}' to '${normalized}'`)
        return normalized
      }
    ...
    const _getFiles = async (filePath, isLocal = false, localFileStats) => {
      if (isLocal) {
        ...
        // is dir
        logger.debug(`recursively listing all files in local dir '${filePath}'`)
  ...
  options.progressCallback=${options.progressCallback && 'yes'}`)
    ...
    logger.debug(`extended destPath: '${extendedDestPath}'`)

    logger.debug(`listing all src files in '${normalizedSrcPath}'`)
    ...
    if (options.noOverwrite) {
      ...
      Object.entries(srcToDestFilesMapping).map(([k, v]) => {
        if (existingDestFiles.has(v)) {
          logger.debug(`not copying '${k}' to '${v}' because of noOverwrite`)
          delete srcToDestFilesMapping[k]
        }
      })
    }
    logger.debug(`copying src to dest using Map: ${srcToDestFilesMapping}`)
