...
class Files {
  ...
  async _wrapProviderRequest (requestPromise, details, filePathToThrowOn404 = null) {
    ...
    return requestPromise
      .then(res => {
        const monitoringDetails = this._getMonitoringDetails(res, undefined)
        logger.debug(JSON.stringify({
          ...copyDetails,
          _internal: monitoringDetails
        }))
        return res
      })
      .catch(e => {
        const status = this._statusFromProviderError(e)
        copyDetails._internal = this._getMonitoringDetails(undefined, e)
  ...
  _statusFromProviderError (e) {
    // remove this one in favor of _getMonitoringDetails
   ...
   * @param {Error} e OR error response
   * @returns {object} with response and request objects
   *
   * @protected
   * @memberof Files
   * @abstract
  ...
  _getMonitoringDetails (res, e) {
    throwNotImplemented('_getMonitoringDetails')
  }
