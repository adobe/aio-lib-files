...
class Files {
   ...
   * @memberof Files
   */
  // error handling is too specific to Azure and needs to be refactored once we support more providers
  async _wrapProviderRequest (requestPromise, details, filePath) {
    ...
    return requestPromise.catch(e => {
      ...
      if (!filePath && status === 404) {
        return null
      }
      if (filePath && status === 404) {
        logAndThrow(new codes.ERROR_FILE_NOT_EXISTS({ messageValues: [filePath], sdkDetails: copyDetails }))
      }
      if (status === 403) {
        logAndThrow(new codes.ERROR_BAD_CREDENTIALS({ messageValues: ['cloud storage provider'], sdkDetails: copyDetails }))
      }
      if (status === 416 && details.options.position !== undefined) {
        logAndThrow(new codes.ERROR_OUT_OF_RANGE({ messageValues: [`options.position ${details.options.position} out of range for file ${filePath}`], sdkDetails: copyDetails }))
      }
...
module.exports = { Files, FilePermissions }
