...
class Files {
  ...
  static _throwIfRemoteDirectory (filePath, details) {
    if (this._isRemoteDirectory(filePath)) throw new errorCodes.ERROR_BAD_FILE_TYPE({ messageValues: [`${filePath} is a directory but should be a file`], sdkDetails: { details } })
  }
   ...
   * @memberof Files
   */
  async _wrapProviderRequest (requestPromise, filePath, details) {
  ...
  async write (filePath, content) {
    const details = { filePath, contentType: content.constructor.name } // don't pass all content to error detail
    validateFilePath(filePath, details)
