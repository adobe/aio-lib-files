  ...
  return promise
    .catch(e => {
      if (e.status === 401 || e.status === 403) {
        throw new codes.ERROR_BAD_CREDENTIALS({ messageValues: ['TVM'], sdkDetails: cloneDeep(details) })
      }
...
async function init (config = {}) {
  ...
  switch (provider) {
    case 'azure':
      return AzureBlobFiles.init(config.azure || (await wrapTVMRequest(tvm.getAzureBlobCredentials(), { ...config.tvm, namespace: config.ow && config.ow.namespace })))
    // default:
