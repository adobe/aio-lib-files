...
async function init (config = {}) {
  ...
  const tvm = await utils.getTVMClient(provider, config)
  ...
  switch (provider) {
    case 'azure':
      if (config.azure) {
        ...
        return AzureBlobFiles.init(config.azure, config)
      }
      const cred = await utils.getAzureBlobCredentials(tvm)
      return AzureBlobFiles.init(cred, config)
    // default:
